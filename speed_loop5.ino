// pwm limitations
const unsigned int max_count = 400;  // defines a operating pwm frequency of 20kHz 400
...
// global variables
unsigned int pwm = 300;
unsigned int dir =  1; // motor direction and speed

unsigned int c_command = 700; // in terms of adc value for the given circuit
signed int c_error[3] = {0,0,0}; // current loop variables

unsigned int v_command = 70; // in terms of count differences in an iteration
...
void phase_a() 
{
  ...
  if ((a==1&&b==0)||(a==0&&b==1)) {pos_count[0]++;}
  if ((a==0&&b==0)||(a==1&&b==1)) {pos_count[0]--;}
}
void phase_b() 
{
  boolean a,b;
  a = digitalRead(encoder_in_a);
  b = digitalRead(encoder_in_b);
  if ((a==1&&b==1)||(a==0&&b==0)) {pos_count[0]++;}
  if ((a==0&&b==1)||(a==1&&b==0)) {pos_count[0]--;}
}
