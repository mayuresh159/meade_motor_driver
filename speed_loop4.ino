// pwm limitations
const unsigned int max_count = 1000;  // defines a operating pwm frequency of 4kHz
const unsigned int pwm_up_sat = 995; 
...
{
//    //loop_repeat++;
//    //if (loop_repeat == 10){v_loop(); loop_repeat=0;}
//    v_loop();
//    c_loop();
//    set_pwm();
//    purge_data();
...
// timer for loop repetition
void init_timer()
{
  sei();
  TCCR2A = _BV(WGM21);
  OCR2A = 200; // set compare match value for the clock with a prescaler of /256 to get a 1 msec interrupt
  TIMSK2 = _BV(OCIE2A) ;
  TCNT2 = 0;
  TCCR2B = _BV(CS22) | _BV(CS21) | _BV(CS20);  // start the timer 
}
